...
def printTotalDistance(total_list):
    
    courierName = ["City-Link", "Pos Laju", "GDEX", "J&T", "DHL"]

    for j in range(len(courierName)):
        print(courierName[j], ": ", total_list[j], " km")

    print()


def courierSorting(total_list):
    
    courierName = ["City-Link", "Pos Laju", "GDEX", "J&T", "DHL"]

    total_list_zip = list(zip(*[total_list, courierName]))
    quickSort(total_list_zip, 0, len(total_list_zip)-1)

    return total_list_zip
...
if __name__ == '__main__':
    ...
    total1_list = courierSorting(total1_list)
    print(total1_list)
    print("The shortest distance parcel travel from Rawang to Bukit Jelutong is ",total1_list[0][0],"km\n")

    printTotalDistance(total2_list)
    total2_list = courierSorting(total2_list)
    print(total2_list)
    print("The shortest distance parcel travel from Subang Jaya to Puncak Alam is ",total2_list[0][0],"km\n")

    printTotalDistance(total3_list)
    total3_list = courierSorting(total3_list)
    print(total3_list)
    print("The shortest distance parcel travel from Ampang to Cyberjaya is ",total3_list[0][0],"km\n")
